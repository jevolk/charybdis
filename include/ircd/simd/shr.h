...
noexcept
{
	...
	static_assert
	(
		b % CHAR_BIT == 0, "[emulated] xmmx register only shifts right at bytewise resolution."
	);
...
noexcept
{
	static_assert
	(
		b % CHAR_BIT == 0, "xmmx register only shifts right at bytewise resolution."
	);

	return T(_mm_bsrli_si128(u128x1(a), b / CHAR_BIT));
}
#else
template<int b,
         class T>
...
noexcept
{
	static_assert
	(
		b % CHAR_BIT == 0, "ymmx register only shifts right at bytewise resolution."
	);

	return T(_mm256_srli_si256(u256x1(a), b / CHAR_BIT));
}
#else
template<int b,
         class T>
